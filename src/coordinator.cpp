...
#include "sim/PlaneCoordinator.h"
...
std::map<int, sim::PlaneCoordinator> planesArray;
...
bool requestPlaneID(sim::RequestPlaneID::Request &req, sim::RequestPlaneID::Response &res)
{
	...
	if(req.requestedID == -1)
	{
		...
		while(true)
		{
			...
			else
			{
				...
				planesArray[id] = sim::PlaneCoordinator();
	...
	else
	{
		...
		{
			planesArray[req.requestedID] = sim::PlaneCoordinator();
...
*/
bool goToWaypoint(sim::GoToWaypoint::Request &req, sim::GoToWaypoint::Response &res)
{
	...
	{
		//construct waypoint
		struct sim::waypoint pointFromService;
		...
		if(planesArray[req.planeID].goToPoint(pointFromService, req.isAvoidanceManeuver, req.isNewQueue))
		{
			...
			*/
			...
			{
				//get the command
				sim::Command commandToSend = planesArray[req.planeID].getPriorityCommand();
...
*/
bool loadPathCallback(sim::LoadPath::Request &req, sim::LoadPath::Response &res)
{
	...
	if(isValidPlaneID(req.planeID))
	{
		//open our file
		FILE *fp;
		fp = fopen((ros::package::getPath("sim")+"/paths/"+req.filename).c_str(), "r");
...
*/
bool loadCourseCallback(sim::LoadCourse::Request &req, sim::LoadCourse::Response &res)
{
	ROS_INFO("Received request: Load course from \"%s\"\n", req.filename.c_str());
	
	//open our file
	FILE *fp;
	fp = fopen((ros::package::getPath("sim")+"/courses/"+req.filename).c_str(), "r");
...
*/
bool requestWaypointInfoCallback(sim::RequestWaypointInfo::Request &req, sim::RequestWaypointInfo::Response &res)
{
	//check that the request ID is valid
	if(isValidPlaneID(req.planeID))
	{
		//fill out the waypoint to return
		sim::waypoint temp = planesArray[req.planeID].getWaypointOfQueue(req.isAvoidanceWaypoint, req.positionInQueue);
...
int main(int argc, char **argv)
{
	...
	commandPub = n.advertise<sim::Command>("commands", 1000);
