...
std::map<int, std::queue<struct sim::waypoint> > mapOfPaths;
...
*/
bool saveFlightData(sim::SaveFlightData::Request &req, sim::SaveFlightData::Response &res)
{
	isMonitoringTelemetry = false;
	ROS_INFO("Saving to file %s...", req.filename.c_str());
	
	FILE *fp = fopen((ros::package::getPath("sim")+"/flightData/"+req.filename).c_str(), "w");
	if(fp != NULL)
	{
		...
		//here goes the magic
		std::map<int, std::queue<struct sim::waypoint> >::iterator ii;
