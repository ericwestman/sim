...
{	
	...
	for (int n = 0; n < numPlanes; n++) {
		...
		*/	
		...
		currentPlaneID = n;
		sim::waypoint newWaypoint;
		std::queue<sim::waypoint> waypointQueue = findNewWaypoint(planes[currentPlaneID], planes);

		while (!waypointQueue.empty()) {
			ROS_WARN("Queue size = %d for plane %d", waypointQueue.size(), currentPlaneID);
			newWaypoint = waypointQueue.front();
			waypointQueue.pop();
