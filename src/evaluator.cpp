...
#define TIME_LIMIT 100 //10 minutes
...
std::map<int, sim::TelemetryUpdate> previousUpdatesMap;
std::map<int, sim::TelemetryUpdate> latestUpdatesMap;

//the list of waypoints to go at
std::map<int, std::queue<sim::waypoint> > waypointQueues;
...
void endEvaluation()
{
	...
	FILE *fp;
	fp = fopen((ros::package::getPath("sim")+"/scores/"+scoresheetFilename+".score").c_str(), "w");
	...
	else
	{
		...
		if(totalMinDist != 0) fprintf(fp, "Total Minimum Distance / Total Distance Traveled%lf\n", totalMinDist/totalDistTraveled);
		...
		while(fgets(buffer, sizeof(buffer), fp))
		{
			...
			else
			{
				...
				if(isFirstPoint.find(planeID) == isFirstPoint.end())
				{
					...
					latestUpdatesMap[planeID] = sim::TelemetryUpdate();
					...
					previousUpdatesMap[planeID] = sim::TelemetryUpdate();
...
void telemetryCallback(const sim::TelemetryUpdate::ConstPtr& msg)
{
	...
	if(msg->planeID == maxAlivePlane)
	{
		...
		struct sim::waypoint current, other;
		...
		for(int id = 0; id <= lastPlaneID; id++)
		{
			...
			while(!waypointQueues[id].empty() && distanceBetween(waypointQueues[id].front(), current) < COLLISION_THRESHOLD)
			{
				...
				struct sim::waypoint temp = waypointQueues[id].front();
