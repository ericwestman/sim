...
#define SIMULATION_TIME 10					// TODO make sure this is the same as in the evaluator
...
int main()
{
	...
	if(fp != NULL)
	{
		...
		/*for ( int i = 0; i < n; i++ ) {
			FILE *original;
			original = fopen("/src/ripna_original.cpp", "r");	

			FILE *ripna;
			ripna = fopen("/src/ripna.cpp", "w");
			
			while (fgets(buffer_2, sizeof(buffer_2), original)) {
				fwrite(buffer_2, 1, sizeof(buffer), ripna);
			}*/	
